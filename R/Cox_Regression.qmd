...
```{r}
cox_model_baseline_glob <- coxph(Surv(Survival_Months, OS_status) ~ 1, data = df) 
cox_model_age_gender_glob <- coxph(Surv(Survival_Months, OS_status) ~ Age_group + Sex, data = df) 
...
```{r}
newdata <- data.frame(
  Age_group = factor("<30", levels = levels(df_gender$Age_group)), 
  Sex = factor(c("Female", "Male"), levels = levels(as.factor(df_gender$Sex)))
)

surv_fit_glob <- survfit(cox_model_age_gender_glob, newdata = newdata)
risk_table_data <- summary(surv_fit_glob)$table
