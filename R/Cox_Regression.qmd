...
```{r}
cox_model_baseline_glob <- coxph(Surv(Survival_Months, OS_status) ~ 1, data = df) 
cox_model_cancer_types_glob <- coxph(Surv(Survival_Months, OS_status) ~ CANCER_TYPE, data = df) 
...
```{r}
cox_model_baseline_glob <- coxph(Surv(Survival_Months, OS_status) ~ 1, data = df_gender) 
cox_model_age_gender_glob <- coxph(Surv(Survival_Months, OS_status) ~ Age_group + Sex, data = df_gender) 
...
```{r}
cox_model_gender_glob <- coxph(Surv(Survival_Months, OS_status) ~ Sex, data = df_gender) 
...
cox_model_glioma_baseline <- coxph(Surv(Survival_Months, OS_status) ~ 1, data = df_glioma)

cox_model_glioma_tumor_purity <- coxph(Surv(Survival_Months, OS_status) ~ TUMOR_PURITY, data = df_glioma) 
...
```{r}
cox_model_glioma_age_gender <- coxph(Surv(Survival_Months, OS_status) ~ Age_group + Sex, data = df_glioma) 
...
```{r}
cox_model_glioma_gender <- coxph(Surv(Survival_Months, OS_status) ~ Sex, data = df_glioma)
