...
```{r}
data_mutation <- read_tsv("/net/pupil1/home/people/s253729/R/r_for_bio_data_science/group23_project/data/data_mutations.txt",
                                 na = c("", "NA", "NULL"),
...
```{r}
#data_mutation_without_nas_long <-  data_mutation_without_nas_long |> 
#  mutate(n_variant = str_count(Variant_Classification,",")+1)

max_variants <- data_mutation_without_nas_long$n_variant |> 
  max()
sprintf("The max number of variants for one row is %.4f",max_variants)

```

So there is only one variant per row.
