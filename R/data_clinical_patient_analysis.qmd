...
Making a survival rate column, using the Kaplan-Meier survival analysis
...
patients |>
  ...
  ggplot(mapping = aes(x = Drug.Type,
                       ...
                       fill = Sex))+
  geom_point()+
  facet_wrap(~ Survival.Status.Clean)
```
...
ggsurvplot(fit, data = patients, legend = "right")
...
fit <- survfit(Surv(Overall.Survival..Months., Overall.Survival.Status) ~ Drug.Type, data = patients)
